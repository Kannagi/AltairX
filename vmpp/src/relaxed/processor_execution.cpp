...
ArResult executeInstruction(ArProcessor processor, uint32_t index)
{
	...
	uint64_t* const ireg = processor->ireg;

	float*    const freg = (float*)processor->freg;
	double*   const dreg = (double*)processor->freg;
	Vector4f* const vreg = (Vector4f*)processor->freg;

	const ArOperation* const op = &processor->operations[index];
	const uint32_t*    const operands = op->operands;
...
ArResult executeDelayedInstruction(ArProcessor processor, uint32_t index)
{
	static const uint32_t ZSUClearMask = ~(Z_MASK | S_MASK | U_MASK);
	static const uint32_t retClearMask = ~R_MASK;

	const ArOperation* op = &processor->delayed[index];
	const uint32_t*    const operands = op->operands;
...
ArResult copyFromRAM(ArProcessor processor, uint64_t ramAddress, uint8_t* output, size_t size)
{
...
ArResult copyToRAM(ArProcessor processor, uint64_t ramAddress, const uint8_t* input, size_t size)
{
	ArPhysicalMemory memory = processor->parent->memory; //First memory
	if(!memory)
	{
...
ArResult executeDMA(ArProcessor processor, int store)
{
	//RAM -> SDRAM
	uint64_t* const ireg = processor->ireg;

	const ArOperation* const op       = &processor->dmaOperation;
	const uint32_t*    const operands = op->operands;
...
ArResult executeDMAR(ArProcessor processor, int store)
{
	//RAM -> SDRAM
	uint64_t* const ireg = processor->ireg;
...
ArResult executeDMAIR(ArProcessor processor)
{
	//RAM -> SDRAM
	uint64_t* const ireg = processor->ireg;
